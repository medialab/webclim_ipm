...
Science Feedback is a fact-checking organization, in which academics are verifying the credibility of science-related claims and articles. 
...
The average percentage change is 7\%, and the median -6\%, both values being very close to zero. 
A Wilcoxon test confirmed that these percentage changes are not significantly different from zero (W = $16051$, p-value = $0.74$).
...
\caption{Metrics aggregated over the 307 Facebook accounts that repeatedly shared false news links. {\bf(Top panel)} Average engagement per post. {\bf(Middel panel)} Number of posts per day. {\bf(Bottom panel)} Engagement per day. The dotted line marks the date of June 9, 2020, when a sudden drop in engagement is observed.}
