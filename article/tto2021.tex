...
{\color{red} If we compare the engagement between the `repeat offender' and the `no strike' periods, we calculated a decrease of X \% during the `repeat offender' periods, which is not blablabla.}
...
{\color{red} We then calculated the percentage change between the `repeat offender' and the `no strike' periods for each of the X Facebook accounts that published at least one post during these periods (see Figure X). 
The average percentage change was -X\% and the median -X\%, and a Wilcoxon test revealed the percentage changes to be not (?) significantly different than zero ($W = X$, $p-value = X$).
We thus found blablabla.

[ADD THE LAST FIGURE HERE]}
