...
%The magnitude of this drop corresponds to a 47\% decrease in the number of reactions per post, a 50\% decrease in the number of shares and a 44\% decrease for comments, calculated as the difference between June 10 and June 8, 2020. We can only explain such a massive change by a modification in how Facebook’s algorithm promotes the content from these groups. If we look at the accounts individually, among the 276 accounts that were active on the 8th and 10th of June, 2020, 216 accounts had a decrease in their total engagement per post.

The results showed a global lack of influence of fact-checks on the engagement metrics of repeat offenders. 
In the following section we chose to focus on the accounts which we are certain are under `repeat offender' status. 
...
If we compare the engagement between 30 days before and 30 days after April 28, 2020, the percentage change is only 3\%, indicating almost no change between these two periods.
...
The average percentage change was -17\% and the median -23\%. 
Although the decrease was small, a Wilcoxon test revealed the percentage changes to be significantly below zero ($W = 912.0$, $p-value = 0.00026$).
...
A limitation of the results is that we only took into account the links labelled as `False' by one fact-checking organization (Science Feedback), while Facebook partners with over 60 fact-checking organizations\footnote{https://about.fb.com/news/2020/04/covid-19-misinfo-update/}.
