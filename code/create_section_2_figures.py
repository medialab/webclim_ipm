...
def import_crowdtangle_group_data_all():
    ...
    for file_index in range(4):
        df_list.append(import_data(file_name="posts_condor_group_" + str(file_index + 1) + ".csv"))
    posts_group_df = pd.concat(df_list)
    print("\nThere are {} 'repeat offender' Facebook groups.".format(posts_group_df.account_id.nunique()))

    posts_page_df_1 = import_data(file_name="posts_condor_page_1.csv")
    posts_page_df_2 = import_data(file_name="posts_condor_page_2.csv")
